...
public class SysLocalDataBase : SysModule
{
	...
	public void UpdateNotifyData(int assisantNum, List<com.kodgames.corgi.protocol.ActivityData> activityData, List<com.kodgames.corgi.protocol.State> functionStates, List<WeihuaGames.ClientClass.Quest> changedQuests, WeihuaGames.ClientClass.QuestQuick questQuick)
	{
		if (localPlayer == null)
			return;
    ...
    private void RegistNotifyFixTimeActivity()
    {
        ...
        //                WeihuaGames.TimeEx.ToLocalDataTime(startTimes[index].Timer).AddMilliseconds(notifCfg.delayTime),
    ...
    private void RegistNotifyStamina()
    {
        ...
        if (staminaFullGenerateTime > 0)
        {
            foreach (var notifCfg in ConfigDatabase.DefaultCfg.LocalNotificationConfig.GetNotificationsByType(LocalNotificationConfig._NotificationType.Stamina))
            {
                ...
                SysNotification.Instance.ScheduleLocalNotification(
                                WeihuaGames.TimeEx.ToLocalDataTime(loginInfo.NowTime).AddMilliseconds(staminaFullGenerateTime).AddMilliseconds(notifCfg.delayTime),
                                notifCfg.messageBody,
                                notifCfg.appIconBadageNumber,
                                notifCfg.hasAction,
                                notifCfg.actionTitle,
    ...
    private void RegistNotifyRecallPlayer()
    {
        ...
        foreach (var notifCfg in ConfigDatabase.DefaultCfg.LocalNotificationConfig.GetNotificationsByType(LocalNotificationConfig._NotificationType.RecallPlayer))
        {
            ...
            SysNotification.Instance.ScheduleLocalNotification(
                            WeihuaGames.TimeEx.ToLocalDataTime(loginInfo.NowTime).AddMilliseconds(notifCfg.delayTime),
                            notifCfg.messageBody,
                            notifCfg.appIconBadageNumber,
                            notifCfg.hasAction,
                            notifCfg.actionTitle,
	...
	{
		WeihuaGames.ClientClass.Goods goods = localPlayer.ShopData.GetGoodsById(goodsId);
		if (goods == null)
			return 0;
	...
	public void RemoveSuperSkillInRewardForShow(WeihuaGames.ClientClass.Reward reward)
	{
	...
	/// </summary>	
	private void UseCost(List<WeihuaGames.ClientClass.Cost> costs, string srcReqName)
	{
	...
	public static List<Pair<int, int>> ConvertIdCountList(List<WeihuaGames.ClientClass.Cost> costs)
	{
		var retList = new List<Pair<int, int>>();
		foreach (WeihuaGames.ClientClass.Cost cost in costs)
	...
	public static string GetCostsDesc(List<WeihuaGames.ClientClass.Cost> costs)
	{
		return GetCostsDesc(ConvertIdCountList(costs), true, false);
	}
	#endregion
	...
	/// </summary>	
	private void AddReward(WeihuaGames.ClientClass.Reward reward, string srcReqName)
	{
		...
		foreach (WeihuaGames.ClientClass.Avatar avatar in reward.Avatar)
		{
		...
		foreach (WeihuaGames.ClientClass.Equipment equip in reward.Equip)
		{
		...
		foreach (WeihuaGames.ClientClass.Skill skill in reward.Skill)
		{
		...
		foreach (WeihuaGames.ClientClass.Dan dan in reward.Dan)
		{
		...
		foreach (WeihuaGames.ClientClass.Beast beast in reward.Beast)
		{
			localPlayer.AddBeast(beast);
			dataChangeds.Add(IDSeg._AssetType.Beast);
		}		

		foreach (WeihuaGames.ClientClass.Consumable consumable in reward.Consumable)
		{
	...
	private static void AddExpriences(WeihuaGames.ClientClass.Player player, int expriences)
	{
	...
	public static void CombineReward(ref List<Pair<int, int>> rewards, WeihuaGames.ClientClass.Reward reward)
	{
		if (reward == null)
			return;
		...
		foreach (var keyValue in SysLocalDataBase.ConvertIdCountList(reward))
		{
			if (IDSeg.ToAssetType(keyValue.first) == IDSeg._AssetType.Item || IDSeg.ToAssetType(keyValue.first) == IDSeg._AssetType.Special)
			{
				...
				if (i >= rewards.Count)
				{
					rewards.Add(new Pair<int, int>(keyValue.first, keyValue.second));
				}
			}
			else
				rewards.Add(new Pair<int, int>(keyValue.first, keyValue.second));
	...
	public static List<Pair<int, int>> ConvertIdCountList(WeihuaGames.ClientClass.Reward reward)
	{
		return ConvertIdCountList(reward, false);
	}

	public static List<Pair<int, int>> ConvertIdCountList(WeihuaGames.ClientClass.Reward reward, bool combineRepeated)
	{
		var retList = new List<Pair<int, int>>();
		if (reward == null)
			return retList;
	...
	public static List<Pair<object, int>> ConvertObjCountList(WeihuaGames.ClientClass.Reward reward)
	{
	...
	public static string GetRewardDesc(WeihuaGames.ClientClass.Reward reward, bool hasRewardTitle)
	{
		return GetRewardDesc(reward, hasRewardTitle, false, false);
	}

	public static string GetRewardDesc(WeihuaGames.ClientClass.Reward reward, bool hasRewardTitle, bool hideCountLessThanOne)
	{
		return GetRewardDesc(reward, hasRewardTitle, hideCountLessThanOne, false);
	}

	public static string GetRewardDesc(WeihuaGames.ClientClass.Reward reward, bool hasRewardTitle, bool hideCountLessThanOne, bool oneLineForOneReward)
	{
	...
	public static string GetRewardFormatDesc(WeihuaGames.ClientClass.Reward reward)
	{
	...
	public static int GetFirstAssetId(WeihuaGames.ClientClass.Reward reward)
	{
	...
	public static List<ClientServerCommon.Reward> CCRewardToCSCReward(WeihuaGames.ClientClass.Reward reward)
	{
		return CCRewardToCSCReward(reward, false);
	}

	public static List<ClientServerCommon.Reward> CCRewardToCSCReward(WeihuaGames.ClientClass.Reward reward, bool mergeSame)
	{
	...
	public static List<com.kodgames.corgi.protocol.ShowReward> CCRewardListToShowReward(WeihuaGames.ClientClass.Reward reward)
	{
	...
	public static List<ClientServerCommon.Reward> CCRewardListToCSCReward(List<WeihuaGames.ClientClass.Reward> rewards)
	{
		return CCRewardListToCSCReward(rewards, false);
	}

	public static List<ClientServerCommon.Reward> CCRewardListToCSCReward(List<WeihuaGames.ClientClass.Reward> rewards, bool mergeSame)
	{
		WeihuaGames.ClientClass.Reward tmpReward = new WeihuaGames.ClientClass.Reward();
    ...
    public void UpdateMassages(List<com.kodgames.corgi.protocol.ChatMessage> msgs)
	{
		...
		WeihuaGames.ClientClass.MsgFlowData msgFlowData = SysLocalDataBase.Inst.LocalPlayer.MsgFlowData;
	...
	public void UpdateQuestData(List<WeihuaGames.ClientClass.Quest> quests, bool syncData)
	{
		if (quests == null)
			return;
		...
		{
			List<WeihuaGames.ClientClass.Quest> deleteQuestDatas = new List<WeihuaGames.ClientClass.Quest>();

			foreach (var quest in quests)
			{
				WeihuaGames.ClientClass.Quest tempQuest = localPlayer.QuestData.GetQuestByQuestID(quest.QuestId);
