...
public class _ProductPublisher
{
	...
	public const int WeihuaGames = 1;
...
public abstract class Platform : MonoBehaviour
{
	...
	public static void CreateOnGameObject(GameObject gameObject)
	{
		...
		//{
		//	case _ProductPublisher.WeihuaGames: instance = gameObject.AddComponent<KodPlatform>(); break;
...
#elif UNITY_IPHONE
			if (WeihuaGames.ExternalCall.KodConfigPlugin.IsAppStore())
				return _PlatformType.IPhone;
			else
				return _PlatformType.IPhoneBreak;
#else
            return _PlatformType.UnKnow;
#endif
        }
	}
	...
	// 登录时从服务器获取的平台信息
	private WeihuaGames.ClientClass.ChannelMessage channelMessage;
	public virtual WeihuaGames.ClientClass.ChannelMessage ChannelMessage
	{
	...
	{
		//WeihuaGames.ExternalCall.AlertDialog.Show(
	...
	//	WeihuaGames.ExternalCall.AlertDialogHelper.ParseOnAlertDialogResponse(message, out pressOKNotCancel);
	//	if (pressOKNotCancel)
	//		this.Destroy();
	//}
	...
	// 获取UDID（有可能平台提供接口）
	public virtual string GetUDID() { return WeihuaGames.ExternalCall.DevicePlugin.GetUDID(); }
