...
public static class ItemInfoUtility
{
	...
	//public static string GetAssetEvalutionDesc(WeihuaGames.ClientClass.Avatar avatar)
	//{
	//	return string.Format(GameUtility.GetUIString("UI_Value"), ExpressionCalculate.GetValue_AvatarEvaluation(avatar));
	//}

	//public static string GetAssetEvalutionDesc(WeihuaGames.ClientClass.Equipment equipment)
	//{
	//	return string.Format(GameUtility.GetUIString("UI_Value"), (int)ExpressionCalculate.GetValue_EquipmentEvaluation(equipment));
	//}

	//public static string GetAssetEvalutionDesc(WeihuaGames.ClientClass.Skill skill)
	//{
	...
	public static string GetAttribDisplayString(int type, double value)
	{
		if (type < _AvatarAttributeType.NUMBER_RATE_SPACE)
			return WeihuaGames.Math.RoundToInt(value).ToString();
		else
			return string.Format("{0}%", GameUtility.FloatToPercentageValue((float)value));
	}

	public static string GetAttribDisplayString(WeihuaGames.ClientClass.PropertyModifier kodModifier)
	{
	...
	public static string GetAttribDisplayString(PropertyModifier modifier)
	{
		...
		else
		{
			...
			{
				int _value = WeihuaGames.Math.RoundToInt(modifier.attributeValue);
				//if (_value >= 0)
	...
	public static string GetItemCountStr(WeihuaGames.ClientClass.Consumable consumable)
	{
		return GetItemCountStr(consumable.Amount);
	}

	public static bool IsAvatarEquipped(WeihuaGames.ClientClass.Avatar avatar)
	{
		if (avatar == null)
			return false;
	...
	public static bool IsAvatarCheered(WeihuaGames.ClientClass.Avatar avatar)
	{
		if (avatar == null)
			return false;
	...
	//public static bool IsEquiped(WeihuaGames.ClientClass.Avatar avatar)
	//{
	//	if (avatar == null)
	//		return false;
	...
	//public static bool IsEquiped(WeihuaGames.ClientClass.Equipment equipment)
	//{
	//	if (equipment == null)
	//		return false;
	...
	//public static bool IsEquiped(WeihuaGames.ClientClass.Skill skill)
	//{
	//	if (skill == null)
	//		return false;
	...
	//public static bool IsEquiped(WeihuaGames.ClientClass.Dan dan)
	//{
	//	if (dan == null)
	//		return false;
	...
	public static WeihuaGames.ClientClass.Consumable FindConsumableById(List<WeihuaGames.ClientClass.Consumable> consumables, int assetId)
	{
		if (consumables != null)
	...
	public static WeihuaGames.ClientClass.Consumable FindConsumableByItemType(List<WeihuaGames.ClientClass.Consumable> consumables, int itemType)
	{
		if (consumables != null)
			foreach (var consumable in consumables)
	...
	//public static string GetSuiteDesc(WeihuaGames.ClientClass.Player player, int resourceId, WeihuaGames.ClientClass.Location location, bool showSuitProcess)
	//{
	...
	//	var equipedEquipments = new List<WeihuaGames.ClientClass.Equipment>();
	//	var equipedSkills = new List<WeihuaGames.ClientClass.Skill>();
	...
	public static List<int> GetAvatarAssembleRequireIds(WeihuaGames.ClientClass.Avatar avatar)
	{
	...
	// Avatar : PowerUp.
	public static bool IsAbilityUpImprove_Avatar(WeihuaGames.ClientClass.Avatar avatarData)
	{
		return IsLevelNotifyActivity_Avatar(avatarData) || IsBreakNotifyActivity_Avatar(avatarData);
	}

	// Avatar : LevelUp.
	public static bool IsLevelNotifyActivity_Avatar(WeihuaGames.ClientClass.Avatar avatarData)
	{
	...
	// Avatar : BreakThough.
	public static bool IsBreakNotifyActivity_Avatar(WeihuaGames.ClientClass.Avatar avatarData)
	{
		if (avatarData == null || string.IsNullOrEmpty(avatarData.Guid) || !avatarData.IsAvatar)
			return false;
		...
		// Item Count.
		WeihuaGames.ClientClass.Consumable consumable = SysLocalDataBase.Inst.LocalPlayer.SearchConsumable(breakThroughConfig.breakThrough.itemCostItemId);
	...
	private static int GetMaxLevelCostCount(WeihuaGames.ClientClass.Avatar avatarData, int fromLevel, int toLevel)
	{
		var avatarConfig = ConfigDatabase.DefaultCfg.AvatarConfig.GetAvatarById(avatarData.ResourceId);
		int costCount = 0;
	...
	private static int GetSameAvatarCount(WeihuaGames.ClientClass.Avatar avatar)
	{
		int sameCount = 0;
		foreach (WeihuaGames.ClientClass.Avatar avatarItem in SysLocalDataBase.Inst.LocalPlayer.Avatars)
		{
	...
	// Dan : PowerUp.
	public static bool IsAbilityUpImprove_Dan(WeihuaGames.ClientClass.Dan dan)
	{
		return IsLevelNotifyActivity_Dan(dan) || IsBreakNotifyActivity_Dan(dan);
	}

	// Dan : LevelUp.
	public static bool IsLevelNotifyActivity_Dan(WeihuaGames.ClientClass.Dan dan)
	{
	...
	public static string GetDanAttributeDesc(WeihuaGames.ClientClass.DanAttributeGroup danAttributeGroup, int danLevel)
	{
		string attr = "";
		//描述
	...
	// Dan : Refine.
	public static bool IsBreakNotifyActivity_Dan(WeihuaGames.ClientClass.Dan dan)
	{
		if (dan == null || string.IsNullOrEmpty(dan.Guid))
			return false;
	...
	// Equipment : PowerUp.
	public static bool IsAbilityUpImprove_Equip(WeihuaGames.ClientClass.Equipment equipment)
	{
		if (equipment == null)
			return false;
	...
	// Equipment : LevelUp.
	public static bool IsLevelNotifyActivity_Equip(WeihuaGames.ClientClass.Equipment equipment)
	{
	...
	// Equipment : Refine.
	public static bool IsBreakNotifyActivity_Equip(WeihuaGames.ClientClass.Equipment equipment, EquipmentConfig.Equipment equipConfig)
	{
		if (equipment == null || string.IsNullOrEmpty(equipment.Guid))
			return false;
	...
	//public static List<string> ExchangeGetItemAvaliableGuids(WeihuaGames.ClientClass.CostAsset costAsset)
	//{
	...
	//public static List<string> ExchangeGetItemAvaliableGuids(WeihuaGames.ClientClass.ItemEx cost)
	//{
	//	List<string> result = new List<string>();
	...
	//public static List<WeihuaGames.ClientClass.Avatar> ExchangeGetAvaliableAvatar(WeihuaGames.ClientClass.ItemEx cost)
	//{
	//	List<WeihuaGames.ClientClass.Avatar> result = new List<WeihuaGames.ClientClass.Avatar>();
	...
	public static List<WeihuaGames.ClientClass.Avatar> ExchangeGetAvaliableAvatar(WeihuaGames.ClientClass.CostAsset cost)
	{
		List<WeihuaGames.ClientClass.Avatar> result = new List<WeihuaGames.ClientClass.Avatar>();
	...
	//public static List<WeihuaGames.ClientClass.Equipment> ExchangeGetAvaliableEquip(WeihuaGames.ClientClass.ItemEx cost)
	//{
	//	List<WeihuaGames.ClientClass.Equipment> result = new List<WeihuaGames.ClientClass.Equipment>();
	...
	//public static List<WeihuaGames.ClientClass.Equipment> ExchangeGetAvaliableEquip(WeihuaGames.ClientClass.CostAsset cost)
	//{
	//	List<WeihuaGames.ClientClass.Equipment> result = new List<WeihuaGames.ClientClass.Equipment>();
	...
	//public static List<WeihuaGames.ClientClass.Skill> ExchangeGetAvaliableSkill(WeihuaGames.ClientClass.ItemEx cost)
	//{
	//	List<WeihuaGames.ClientClass.Skill> result = new List<WeihuaGames.ClientClass.Skill>();
	...
	//public static List<WeihuaGames.ClientClass.Skill> ExchangeGetAvaliableSkill(WeihuaGames.ClientClass.CostAsset cost)
	//{
	//	List<WeihuaGames.ClientClass.Skill> result = new List<WeihuaGames.ClientClass.Skill>();
	...
	//public static List<WeihuaGames.ClientClass.Dan> ExchangeGetAvailiableDan(WeihuaGames.ClientClass.ItemEx cost)
	//{
	//	List<WeihuaGames.ClientClass.Dan> result = new List<WeihuaGames.ClientClass.Dan>();
	...
	//public static List<WeihuaGames.ClientClass.Dan> ExchangeGetAvailiableDan(WeihuaGames.ClientClass.CostAsset cost)
	//{
	//	List<WeihuaGames.ClientClass.Dan> result = new List<WeihuaGames.ClientClass.Dan>();
	...
	{
		// 获得cost的Id
		if (costOption is WeihuaGames.ClientClass.ItemEx)
			return IDSeg.ToAssetType((costOption as WeihuaGames.ClientClass.ItemEx).Id);

		if (costOption is WeihuaGames.ClientClass.CostAsset)
			return (costOption as WeihuaGames.ClientClass.CostAsset).Type;
	...
	public static int ExchangeGetCostCount(object costOption)
	{
		//获取 cost count
		if (costOption is WeihuaGames.ClientClass.ItemEx)
			return (costOption as WeihuaGames.ClientClass.ItemEx).Count;

		if (costOption is WeihuaGames.ClientClass.CostAsset)
			return (costOption as WeihuaGames.ClientClass.CostAsset).Count;
	...
	public static string ExchangeGetCostName(object costOption)
	{
		...
		if (costOption is WeihuaGames.ClientClass.ItemEx)
		{
			var clientCost = costOption as WeihuaGames.ClientClass.ItemEx;
		...
		if (costOption is WeihuaGames.ClientClass.CostAsset)
		{
			var costAsset = costOption as WeihuaGames.ClientClass.CostAsset;
			...
			if (!string.IsNullOrEmpty(subTypeName))
			{
				if (costAsset.Type == IDSeg._AssetType.Dan)
					...
					return GameUtility.FormatUIString("UIPnlChooseCard_CostName2",
								ItemInfoUtility.GetAssetQualityLevelCNDesc((costOption as WeihuaGames.ClientClass.CostAsset).QualityLevel),
								subTypeName,
								name);
			}
			else
			{
				if ((costAsset.Type != IDSeg._AssetType.Dan))
					return GameUtility.FormatUIString("UIPnlChooseCard_CostName", ItemInfoUtility.GetAssetQualityLevelCNDesc((costOption as WeihuaGames.ClientClass.CostAsset).QualityLevel), name);
				else
					return name;
			}
		}
	...
	public static string ExchangeGetBreakLabelDesc(object costOption)
	{
		...
		{
			int minBreakLevel = (costOption is WeihuaGames.ClientClass.ItemEx) ? (costOption as WeihuaGames.ClientClass.ItemEx).ExtensionBreakThroughLevelFrom : (costOption as WeihuaGames.ClientClass.CostAsset).BreakThroughLevelFrom;
			int maxBreakLevel = (costOption is WeihuaGames.ClientClass.ItemEx) ? (costOption as WeihuaGames.ClientClass.ItemEx).ExtensionBreakThroughLevelTo : (costOption as WeihuaGames.ClientClass.CostAsset).BreakThroughLevelTo;
			int resourceType = (costOption is WeihuaGames.ClientClass.ItemEx) ? IDSeg.ToAssetType((costOption as WeihuaGames.ClientClass.ItemEx).Id) : (costOption as WeihuaGames.ClientClass.CostAsset).Type;
